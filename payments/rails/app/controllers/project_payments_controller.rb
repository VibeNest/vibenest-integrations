class ProjectPaymentsController < ApplicationController
  ...
  def webhook
    return head(:unsupported_media_type) if request.headers["Content-Encoding"].present?
    service.accept_webhook(request.raw_post, request.headers["Paddle-Signature"].to_s)
  ...
  def verifier
    return head(:not_found) unless ENV["VIBENEST_PROJECT_PAYMENTS_VERIFIER_ENABLED"] == "true" && ActiveSupport::SecurityUtils.secure_compare(ENV.fetch("VIBENEST_PROJECT_PAYMENTS_WEBHOOK_SECRET"), request.headers["X-VibeNest-Simulator-Secret"].to_s)
    render json: { provider: "simulator", environmentId: ENV["VIBENEST_PROJECT_PAYMENTS_ENVIRONMENT_ID"], manifestDigest: ENV["VIBENEST_PROJECT_PAYMENTS_MANIFEST_DIGEST"], builtCommit: ENV["SOURCE_COMMIT"] }
  end
