...
import { assertCsrf, authSettings, installVibeNestAuth } from "./vibenest-auth.mjs";
import {
  acceptSignedEvent,
...
export function createApp(configuration = process.env, dependencies = {}) {
  ...
  const authEnabled = configuration.VIBENEST_AUTH_ENABLED === "true";
  if (authEnabled) {
    app.set("trust proxy", configuration.TRUSTED_PROXY_CIDR ?? "loopback");
    app.use(session({
      name: "vibenest_app_session",
      secret: requireSessionSecret(configuration.APP_SESSION_SECRET),
      store: dependencies.sessionStore,
      resave: false,
      saveUninitialized: false,
      cookie: { httpOnly: true, secure: true, sameSite: "lax", maxAge: 8 * 60 * 60_000 }
    }));
    installVibeNestAuth(app, configuration, dependencies.oidcApi);
  }
  ...
  const resolveAuthenticatedIdentity = dependencies.resolveAuthenticatedIdentity
    ?? (authEnabled
      ? request => vibenestSessionIdentityResolver(request, configuration)
      : configuration.VIBENEST_FIXTURE_AUTH_ENABLED === "true"
...
export function vibenestSessionIdentityResolver(request, configuration = process.env) {
  const subjectKey = request.session?.user?.subject;
  if (typeof subjectKey !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/.test(subjectKey)) return null;
  assertCsrf(request, authSettings(configuration).applicationOrigin);
  return { subjectKey };
}
...
function requireSessionSecret(value) {
  if (typeof value !== "string" || value.length < 32) throw new Error("APP_SESSION_SECRET must contain at least 32 characters.");
  return value;
}
