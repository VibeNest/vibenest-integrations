...
export function installVibeNestAuth(app, environment = process.env, api = oidc) {
  const value = authSettings(environment);
  ...
  app.get("/auth/vibenest/login", async (request, response, next) => {
    try {
      ...
      request.session.oidc = { codeVerifier, state, nonce, expiresAt: Date.now() + 300_000 };
      ...
      request.session.save((error) => (error ? next(error) : response.redirect(303, authorizationUrl.href)));
  ...
  app.get("/auth/vibenest/callback", async (request, response, next) => {
    try {
      ...
      if (!pending || pending.expiresAt < Date.now() || !pending.codeVerifier || !pending.state || !pending.nonce) {
        return response.sendStatus(400);
      }
      const callbackUrl = callbackUrlFromRequest(request.originalUrl, environment);
      ...
      request.session.regenerate((error) => {
        ...
        request.session.user = { subject: claims.sub, email: profile.email ?? claims.email };
        request.session.csrf = randomBytes(24).toString("base64url");
        request.session.save((saveError) => (saveError ? next(saveError) : response.redirect(303, value.applicationOrigin)));
      });
    } catch (error) { next(error); }
  });

  app.post("/auth/logout", requireUser, requireCsrf(value.applicationOrigin), (request, response, next) => {
    request.session.destroy((error) => (error ? next(error) : response.redirect(303, value.applicationOrigin)));
  });

  app.get("/api/session", requireUser, (request, response) => {
    response.json({ authenticated: true, subject: request.session.user.subject, email: request.session.user.email, csrf: request.session.csrf });
  });
}
...
export function requireCsrf(applicationOrigin) {
  return (request, response, next) => {
    try { assertCsrf(request, applicationOrigin); next(); }
    catch { response.sendStatus(403); }
  };
}

export function assertCsrf(request, applicationOrigin) {
  const expected = request.session?.csrf;
  const supplied = request.get("X-CSRF-Token");
  if (request.get("Origin") !== new URL(applicationOrigin).origin || !safeEqual(expected, supplied)) throw new Error("invalid csrf");
}
