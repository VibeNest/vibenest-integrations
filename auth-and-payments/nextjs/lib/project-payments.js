...
import { readSession, requireCsrf } from "./session.js";
import { settings as authSettings } from "./vibenest-auth.js";
...
const sessionCookieName = "vibenest_fixture_session";
const runtimeInstanceSymbol = Symbol.for("vibenest.project-payments.runtime-instance");
const runtimeInstanceId = globalThis[runtimeInstanceSymbol] ?? randomUUID();
globalThis[runtimeInstanceSymbol] = runtimeInstanceId;
...
export function computeRuntimeEvidenceScope(environmentId, manifestDigest, builtCommit) {
  ...
  if (!digestPattern.test(manifestDigest ?? "")) throw new Error("The evidence manifest digest is invalid.");
  if (!commitPattern.test(builtCommit ?? "")) throw new Error("The evidence build commit is invalid.");
...
export class PaddleProjectPaymentProvider {
  ...
  async previewPrices() { throw new Error("Project payments are disabled: live provider access is unavailable."); }
  async createCheckout() { throw new Error("Project payments are disabled: live provider access is unavailable."); }
  async createPortal() { throw new Error("Project payments are disabled: live provider access is unavailable."); }
}
...
export class SimulatorProjectPaymentProvider {
  ...
  constructor({ catalog, store }) {
    ...
    if (!(store instanceof ProjectPaymentSqliteStore)) throw new Error("A durable Project Payments store is required.");
  ...
  async previewPrices(priceKeys) {
    ...
    this.store.recordProviderEvidence(providerEvidenceAuthority, "price-preview");
    return prices;
  }
  ...
  async createCheckout({ buyerKey, priceKey }) {
    ...
    this.store.bindCustomer(buyerKey, buyerKey);
    return {
      checkoutUrl: `https://simulator.invalid/checkout/${stableId("sim_checkout", buyerKey, priceKey)}`,
  ...
  async createPortal(buyerKey) {
    requireOpaqueIdentifier(buyerKey, "Authenticated buyer");
    this.store.bindCustomer(buyerKey, buyerKey);
    this.store.recordProviderEvidence(providerEvidenceAuthority, "customer-portal");
    return {
...
export function parseRuntimeCatalog(encoded, { expectedEnvironmentId, expectedManifestDigest }) {
  ...
  if (!digestPattern.test(expectedManifestDigest ?? "")) throw new Error("The installed manifest digest is invalid.");
  ...
  let projection;
  try { projection = JSON.parse(strictUtf8.decode(bytes)); }
  catch { throw new Error("The simulator runtime catalog must contain valid UTF-8 JSON."); }
  ...
  if (projection.provider !== providerModes.simulator) throw new Error("The runtime catalog is not a simulator catalog.");
  ...
  if (!safeEqualText(projection.environmentExternalId, expectedEnvironmentId))
    throw new Error("The runtime catalog targets another environment.");
  if (!digestPattern.test(projection.manifestDigest ?? "")
  ...
  const externalPriceIds = new Set();
  for (const product of projection.products) {
    ...
    if (Object.hasOwn(products, product.manifestKey) || externalProductIds.has(product.externalId))
      throw new Error("Runtime product keys and external ids must be unique.");
    externalProductIds.add(product.externalId);
    ...
    for (const price of product.prices) {
      ...
      if (Object.hasOwn(prices, price.manifestKey) || externalPriceIds.has(price.externalId))
        throw new Error("Runtime price keys and external ids must be unique.");
      externalPriceIds.add(price.externalId);
    ...
    products[product.manifestKey] = { providerProductId: product.externalId, prices, grants };
  }
  const catalog = {
    products,
...
export class ProjectPaymentSqliteStore {
  constructor(databasePath, {
    ...
    if (typeof databasePath !== "string" || databasePath.trim() === "") throw new Error("A SQLite path is required.");
    ...
    if (evidenceScope !== null && !digestPattern.test(evidenceScope))
      throw new Error("The lifecycle evidence scope must be a manifest digest.");
  ...
  bindCustomer(providerCustomerId, subjectKey) {
  ...
  resolveSubject(providerCustomerId) {
    requireOpaqueIdentifier(providerCustomerId, "Provider customer id");
  ...
  createFixtureSession(subjectKey, { token = `vnpfs_${randomBytes(32).toString("base64url")}`, expiresAt } = {}) {
    requireOpaqueIdentifier(subjectKey, "Authenticated subject");
    if (!/^vnpfs_[A-Za-z0-9_-]{43}$/.test(token)) throw new Error("The fixture session token is invalid.");
    const expiration = expiresAt ?? new Date(Date.now() + 60 * 60_000);
    if (!(expiration instanceof Date) || !Number.isFinite(expiration.getTime()) || expiration.getTime() <= Date.now())
      throw new Error("The fixture session expiration is invalid.");
    this.db.prepare(`
      INSERT INTO project_payment_auth_session(token_hash, subject_key, expires_at, created_at)
      VALUES (?, ?, ?, ?)
    `).run(hash(token), subjectKey, expiration.toISOString(), new Date().toISOString());
    return token;
  }

  resolveFixtureSession(token, now = new Date()) {
    if (typeof token !== "string" || !/^vnpfs_[A-Za-z0-9_-]{43}$/.test(token)) return null;
    const row = this.db.prepare(`
      SELECT subject_key FROM project_payment_auth_session
      WHERE token_hash = ? AND revoked_at IS NULL AND expires_at > ?
    `).get(hash(token), now.toISOString());
    return row?.subject_key ?? null;
  }

  insertVerifiedEvent(event, receivedAt = new Date()) {
  ...
  stageRestartReplayProbe(destinationKey, now = new Date()) {
    requireOpaqueIdentifier(destinationKey, "Restart replay destination");
    if (!digestPattern.test(this.evidenceScope ?? ""))
    ...
    const event = {
      ...
      occurredAt: stagedAt,
      bodyDigest: hash(`restart-replay\0${destinationKey}\0${this.evidenceScope}\0${this.instanceId}`),
  ...
  processDue({ now = new Date(), maxEvents = 100 } = {}) {
    ...
    for (let index = 0; index < maxEvents; index += 1) {
      const claim = this.claimDue(nowDate);
      if (!claim) break;
      try {
        ...
        if (outcome === "applied" || outcome === "observed") result.applied += 1;
          ...
          result.deferred += 1;
          this.#releaseClaim(claim, nowDate, "A referenced projection is not available yet.");
        }
      } catch (error) {
        result.failed += 1;
        if (this.#releaseClaim(claim, nowDate, "Project Payments event processing failed.")) result.deadLettered += 1;
      }
    }
    return result;
  }

  recordProviderEvidence(authority, name, observedAt = new Date()) {
    if (authority !== providerEvidenceAuthority || !["price-preview", "customer-portal"].includes(name))
      throw new Error("Lifecycle evidence must come from a trusted provider operation.");
    const time = requireOperationalDate(observedAt, "Evidence time").toISOString();
    if (!this.evidenceScope) throw new Error("Trusted provider evidence requires a manifest digest scope.");
    this.#transaction(() => this.#recordEvidence(this.evidenceScope, name, null, time));
  }

  hasEntitlement(subjectKey, grantKey, now = new Date()) {
    requireOpaqueIdentifier(subjectKey, "Authenticated subject");
    requireOpaqueIdentifier(grantKey, "Grant key");
    const row = this.db.prepare(`
    ...
    return row.effective_until === null || Date.parse(row.effective_until) > requireOperationalDate(now, "Entitlement time").getTime();
  }

  verificationStatus(evidenceScope = this.evidenceScope) {
    if (!digestPattern.test(evidenceScope ?? ""))
      return { lifecyclePassed: false, durableInbox: false, restartReplayPassed: false };
    const evidence = new Set(this.db.prepare(
    ...
    `).get();
    return {
      lifecyclePassed: requiredLifecycleEvidence.every(name => evidence.has(name)),
      durableInbox: Number(counts.total) > 0 && Number(counts.pending ?? 0) === 0 && Number(counts.dead ?? 0) === 0,
      restartReplayPassed: evidence.has("restart-replay")
    };
  }

  snapshot() {
    const rows = table => this.db.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all();
    return {
      events: rows("project_payment_webhook_event"),
      entitlements: rows("project_payment_entitlement"),
      entitlementSources: rows("project_payment_entitlement_source"),
      subscriptions: rows("project_payment_subscription"),
      transactions: rows("project_payment_transaction"),
      evidence: rows("project_payment_evidence")
    };
  }
    ...
    switch (event.effectKind) {
      ...
      case "subscription-created": {
        ...
        if (current) {
          ...
          const ordering = compareEventWithRow(event, current);
          if (ordering < 0) return "stale";
        ...
        if (priorTransaction) this.#assertProjectionIdentity(priorTransaction, event, "subscription");
      ...
      case "adjustment-approved": {
        ...
        if (Number(transaction.amount_minor) !== event.amountMinor || transaction.currency !== event.currency)
          throw new InvalidSimulatorEventError("The approved refund does not match its trusted transaction.");
        const resolved = {
          ...event,
  ...
  #upsertTransaction(event) {
    ...
    this.db.prepare(`
      INSERT INTO project_payment_transaction(
        provider_transaction_id, subject_key, grants_json, price_key, price_type,
        provider_subscription_id, amount_minor, currency, billing_period_start,
        billing_period_end, occurred_at, source_event_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(provider_transaction_id) DO UPDATE SET
        subject_key = excluded.subject_key, grants_json = excluded.grants_json,
  ...
  #upsertSubscription(event, status, effectiveUntil) {
    ...
    this.db.prepare(`
      INSERT INTO project_payment_subscription(
        provider_subscription_id, subject_key, grants_json, price_key, status,
  ...
  #validateTransactionContinuity(event) {
    ...
    if (!current) {
      const priorTransaction = this.db.prepare(`
        ...
        ORDER BY occurred_at LIMIT 1
      `).get(event.providerSubscriptionId);
      if (priorTransaction) this.#assertProjectionIdentity(priorTransaction, event, "recurring transaction");
      return null;
    }
    this.#assertSubscriptionIdentity(current, event);
    ...
    if (comparison > 0 && Date.parse(event.billingPeriod.startsAt) !== Date.parse(current.billing_period_end))
      throw new InvalidSimulatorEventError("The recurring transaction period is discontinuous.");
    return null;
  }
  ...
  #validateSubscriptionContinuity(event, mode) {
    ...
    if (mode === "advance") {
      if (comparison === 0) return "stale";
      if (current.status !== "active") throw new InvalidSimulatorEventError("A renewal requires an active subscription.");
      if (Date.parse(event.billingPeriod.startsAt) !== Date.parse(current.billing_period_end))
        throw new InvalidSimulatorEventError("The renewal billing period is discontinuous.");
      return null;
    }
    if (comparison > 0) throw new InvalidSimulatorEventError("Cancellation must reuse the persisted billing period.");
    if (mode === "reuse" && current.status !== "active")
      throw new InvalidSimulatorEventError("Scheduled cancellation requires an active subscription.");
    if (mode === "reuse-for-cancel" && !["active", "scheduled_cancel"].includes(current.status))
      throw new InvalidSimulatorEventError("Immediate cancellation requires active access.");
    return null;
  }
  ...
  #assertProjectionIdentity(row, event, label) {
    if (row.subject_key !== event.subjectKey
  ...
  #recordEvidence(scopeDigest, name, sourceEventId, observedAt) {
    if (!digestPattern.test(scopeDigest ?? "")) throw new Error("Lifecycle evidence requires a manifest digest scope.");
  ...
  #migrate() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS project_payment_auth_session (
...
export function signEvent(rawBody, secret, timestamp) {
  ...
  if (!Number.isSafeInteger(timestamp)) throw new Error("A safe signature timestamp is required.");
  return `ts=${timestamp};h1=${eventDigest(rawBody, secret, timestamp).toString("hex")}`;
}
...
export function verifyEventSignature(rawBody, signature, secret, nowSeconds, toleranceSeconds = 300) {
  ...
  return parsed.digests.some(digest => digest.length === expected.length && timingSafeEqual(digest, expected));
}
...
export async function acceptSignedEvent({
  ...
  requireOpaqueIdentifier(expectedEnvironmentId, "Expected environment id");
  if (!digestPattern.test(evidenceScope ?? "")) throw new Error("The simulator evidence scope is invalid.");
  validateCatalog(catalog);
  if (!(store instanceof ProjectPaymentSqliteStore)) throw new Error("A durable Project Payments store is required.");
  requireRawBuffer(rawBody);
  ...
  if (!verifyEventSignature(rawBody, signature, secret, nowSeconds)) throw new InvalidSimulatorSignatureError();
...
export async function buildVerifierPayload({
  ...
  if (!commitPattern.test(builtCommit ?? "") || !digestPattern.test(installedManifestDigest ?? "")) return null;
  if (!safeEqualText(expectedCommit ?? "", builtCommit) || !safeEqualText(expectedManifestDigest ?? "", installedManifestDigest))
    return null;
  if (!(store instanceof ProjectPaymentSqliteStore)) return null;
  let expectedScope;
  ...
  const evidence = store.verificationStatus(expectedScope);
...
export function isSimulatorHarnessAuthorized({
  ...
  return provider === providerModes.simulator
    ...
    && commitPattern.test(builtCommit ?? "")
    && digestPattern.test(installedManifestDigest ?? "")
    && safeEqualText(expectedCommit ?? "", builtCommit)
    && safeEqualText(expectedManifestDigest ?? "", installedManifestDigest);
}

export function openProjectPaymentRuntime(environment = process.env, options = {}) {
  const enabled = environment.VIBENEST_PROJECT_PAYMENTS_ENABLED === "true";
  const providerMode = environment.VIBENEST_PROJECT_PAYMENTS_PROVIDER;
  if (!enabled || providerMode === providerModes.disabled) {
    return { enabled: false, providerMode: providerModes.disabled, provider: new DisabledProjectPaymentProvider(), close() {} };
  }
  if (providerMode === providerModes.paddle) {
    return { enabled: false, providerMode: providerModes.paddle, provider: new PaddleProjectPaymentProvider(), close() {} };
  }
  if (providerMode !== providerModes.simulator) throw new Error("The Project Payments provider mode is invalid.");
  const secret = environment.VIBENEST_PROJECT_PAYMENTS_WEBHOOK_SECRET;
  requireStrongSecret(secret);
  const environmentId = requireOpaqueIdentifier(
    environment.VIBENEST_PROJECT_PAYMENTS_ENVIRONMENT_ID,
    "Runtime environment id"
  );
  const manifestDigest = environment.VIBENEST_PROJECT_PAYMENTS_MANIFEST_DIGEST;
  const builtCommit = environment.SOURCE_COMMIT;
  const evidenceScope = computeRuntimeEvidenceScope(environmentId, manifestDigest, builtCommit);
  const catalog = parseRuntimeCatalog(environment.VIBENEST_PROJECT_PAYMENTS_CATALOG_B64, {
    expectedEnvironmentId: environmentId,
    expectedManifestDigest: manifestDigest
  });
  const store = new ProjectPaymentSqliteStore(
    options.databasePath ?? environment.PROJECT_PAYMENT_DATABASE_PATH ?? ".data/project-payments.sqlite",
    { instanceId: runtimeInstanceId, ...options.storeOptions, evidenceScope }
  );
  // App Router has no process-global hosted worker contract. Reclaim due/expired leases on
  // every server-side runtime open, then let webhook delivery perform an immediate second
  // sweep after its durable insert. This makes the first request after a restart recover
  // already acknowledged events without relying on an in-memory queue.
  if (options.recoverDue !== false) store.processDue();
  return {
    enabled: true,
    providerMode,
    environmentId,
    ...
    catalog,
    store,
    provider: new SimulatorProjectPaymentProvider({ catalog, store }),
    close() { store.close(); }
  };
}

export function authenticatedBuyerFromRequest(request, store, now = new Date()) {
  const cookieHeader = request.headers.get("cookie") ?? "";
  const applicationSession = readSession(cookieHeader, process.env.APP_SESSION_SECRET, now.getTime());
  if (applicationSession.subject) {
    requireCsrf(request, applicationSession, authSettings().applicationOrigin);
    return applicationSession.subject;
  }
  if (process.env.VIBENEST_FIXTURE_AUTH_ENABLED !== "true") return null;
  const tokens = cookieHeader.split(";").map(part => part.trim()).filter(Boolean);
  const cookie = tokens.find(part => part.startsWith(`${sessionCookieName}=`));
  if (!cookie) return null;
  const token = decodeURIComponent(cookie.slice(sessionCookieName.length + 1));
  return store.resolveFixtureSession(token, now);
}

export function fixtureSessionCookie(token) {
  if (!/^vnpfs_[A-Za-z0-9_-]{43}$/.test(token)) throw new Error("The fixture session token is invalid.");
  return `${sessionCookieName}=${encodeURIComponent(token)}; HttpOnly; Secure; SameSite=Strict; Path=/`;
}

export function parseExactJsonObject(rawText, allowedKeys) {
  let value;
  try { value = JSON.parse(rawText); }
  catch { throw new Error("The request body is invalid JSON."); }
  if (!isPlainObject(value)) throw new Error("The request body must be an object.");
...
export async function readBoundedRawBody(request, maximumBytes = maximumWebhookBytes) {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 1)
    throw new Error("The webhook body limit is invalid.");
  const contentLength = request.headers.get("content-length");
  if (contentLength !== null) {
    if (!/^\d{1,12}$/.test(contentLength) || Number(contentLength) > maximumBytes)
      throw new InvalidSimulatorEventError("The signed simulator event exceeds the accepted size.");
  }
  if (!request.body) throw new InvalidSimulatorEventError("The signed simulator event body is missing.");
  ...
  try {
    while (true) {
      ...
      if (!(value instanceof Uint8Array))
        throw new InvalidSimulatorEventError("The signed simulator event body is invalid.");
  ...
  if (totalBytes === 0) throw new InvalidSimulatorEventError("The signed simulator event body is missing.");
...
async function normalizeSimulatorEvent(parsed, context) {
  ...
  const base = {
    ...
    manifestDigest: context.catalog.manifestDigest,
    evidenceScope: context.evidenceScope
  };
  switch (eventType) {
    case "transaction.completed":
      return { ...base, ...normalizeTransaction(parsed.data, context, "completed", "transaction-completed") };
    case "transaction.payment_failed":
      return { ...base, ...normalizeTransaction(parsed.data, context, "declined", "transaction-declined") };
    case "subscription.created":
      return { ...base, ...normalizeSubscription(parsed.data, context, "active", "subscription-created") };
    case "subscription.updated":
      return { ...base, ...normalizeSubscription(parsed.data, context, "active", "subscription-updated") };
    case "subscription.canceled":
      return { ...base, ...normalizeSubscription(parsed.data, context, "canceled", "subscription-canceled") };
    ...
    case "customer.portal_session.created":
      return { ...base, ...normalizePortal(parsed.data, context) };
    default:
      throw invalid(`Unsupported simulator event type: ${eventType}`);
  }
}

function normalizeTransaction(data, context, expectedStatus, effectKind) {
  requireExactObject(data, ["id", "status", "customer_id", "subscription_id", "items", "details", "custom_data"], "transaction data");
  if (data.status !== expectedStatus) throw invalid("The transaction status is invalid for its event type.");
  const providerCustomerId = requireIdentifier(data.customer_id, "customer id");
  const subjectKey = requireBoundSubject(context.store, providerCustomerId);
  const environmentId = requireEnvironment(data.custom_data, context.expectedEnvironmentId);
  const item = requireSingleItem(data.items, true);
  ...
  const providerSubscriptionId = data.subscription_id === null ? null : requireIdentifier(data.subscription_id, "subscription id");
  ...
  return {
    effectKind,
    environmentId,
    providerTransactionId: requireIdentifier(data.id, "transaction id"),
...
function normalizeSubscription(data, context, expectedStatus, effectKind) {
  requireExactObject(
    data,
    ["id", "status", "customer_id", "items", "current_billing_period", "scheduled_change", "custom_data"],
    "subscription data"
  );
  if (data.status !== expectedStatus) throw invalid("The subscription status is invalid for its event type.");
  const providerCustomerId = requireIdentifier(data.customer_id, "customer id");
  const subjectKey = requireBoundSubject(context.store, providerCustomerId);
  const environmentId = requireEnvironment(data.custom_data, context.expectedEnvironmentId);
  const item = requireSingleItem(data.items, false);
  const catalogPrice = resolveCatalogProviderItem(context.catalog, item.product_id, item.price_id);
  ...
  if (data.scheduled_change !== null) {
    ...
    if (data.scheduled_change.action !== "cancel") throw invalid("Only scheduled cancellation is supported.");
  ...
  if (effectKind === "subscription-created" && data.scheduled_change !== null)
    throw invalid("A new subscription cannot contain a scheduled change.");
  ...
  if (effectKind === "subscription-updated" && effectiveUntil
    ...
    throw invalid("Scheduled cancellation must remain active through the signed period end.");
  }
  return {
    effectKind: effectKind === "subscription-updated" && effectiveUntil ? "subscription-scheduled-cancel" : effectKind,
    environmentId,
    providerSubscriptionId: requireIdentifier(data.id, "subscription id"),
    providerCustomerId,
    subjectKey,
    grants: catalogPrice.grants,
    priceKey: catalogPrice.priceKey,
...
function normalizeAdjustment(data, context, expectedStatus, effectKind) {
  ...
  if (data.action !== "refund" || data.status !== expectedStatus)
    throw invalid("The adjustment action or status is invalid for its event type.");
  const totals = readTotals(data.totals);
  return {
    effectKind,
    environmentId: requireEnvironment(data.custom_data, context.expectedEnvironmentId),
...
function normalizePortal(data, context) {
  requireExactObject(data, ["id", "customer_id", "url", "expires_at", "custom_data"], "portal data");
  const providerCustomerId = requireIdentifier(data.customer_id, "customer id");
  const subjectKey = requireBoundSubject(context.store, providerCustomerId);
  const environmentId = requireEnvironment(data.custom_data, context.expectedEnvironmentId);
  requireIdentifier(data.id, "portal id");
  requireTimestamp(data.expires_at, "portal expires_at");
  let url;
  try { url = new URL(data.url); } catch { throw invalid("The portal URL is invalid."); }
  if (url.protocol !== "https:" || url.hostname !== "simulator.invalid") throw invalid("The portal URL is not a simulator URL.");
  return { effectKind: "portal-created", environmentId, providerCustomerId, subjectKey };
}
...
function validateNormalizedEvent(event) {
  ...
  if (!digestPattern.test(event.bodyDigest ?? "")) throw invalid("The normalized body digest is invalid.");
  if (!digestPattern.test(event.manifestDigest ?? "")) throw invalid("The normalized manifest digest is invalid.");
  if (!digestPattern.test(event.evidenceScope ?? "")) throw invalid("The normalized evidence scope is invalid.");
}

function parseSignedJson(rawBody) {
  requireRawBuffer(rawBody);
  let text;
  try { text = strictUtf8.decode(rawBody); } catch { throw invalid("The signed event is not valid UTF-8."); }
  try { return JSON.parse(text); } catch { throw invalid("The signed event is not valid JSON."); }
}
...
function validateCatalog(catalog) {
  ...
  for (const [productKey, product] of Object.entries(catalog.products)) {
    ...
    if (!Array.isArray(product.grants) || product.grants.length === 0) throw new Error("Catalog grants are required.");
    ...
    for (const grant of product.grants) {
      ...
      if (!Number.isSafeInteger(grant.quantity) || grant.quantity <= 0) throw new Error("Catalog grant quantity is invalid.");
    }
    if (!isPlainObject(product.prices) || Object.keys(product.prices).length === 0) throw new Error("Catalog prices are required.");
    for (const [priceKey, price] of Object.entries(product.prices)) {
      ...
      const pair = `${product.providerProductId}:${price.providerPriceId}`;
      if (providerPairs.has(pair)) throw new Error("Provider catalog mappings must be unique.");
      providerPairs.add(pair);
...
function resolveCatalogPriceByKey(catalog, priceKey) {
  ...
  if (matches.length !== 1) throw new Error("The trusted catalog price key is unknown or ambiguous.");
  return matches[0];
}

function resolveCatalogProviderItem(catalog, productId, priceId) {
  const matches = [];
  for (const [productKey, product] of Object.entries(catalog.products)) {
    for (const [priceKey, price] of Object.entries(product.prices)) {
      if (product.providerProductId === productId && price.providerPriceId === priceId)
...
function requireBillingPeriod(value, price, label) {
  ...
  if (Date.parse(endsAt) !== addBillingInterval(Date.parse(startsAt), price.interval))
...
function requireEnvironment(customData, expectedEnvironmentId) {
  ...
  if (!safeEqualText(environmentId, expectedEnvironmentId)) throw invalid("The signed event targets another environment.");
...
function requireSingleItem(items, includesBillingPeriod) {
  ...
  if (!Number.isSafeInteger(items[0].quantity) || items[0].quantity <= 0) throw invalid("The item quantity is invalid.");
  return items[0];
}

function validateTotals(value, price, quantity) {
  const totals = readTotals(value);
  if (totals.currency !== price.currency || totals.amountMinor !== price.unitAmount * quantity)
...
function readTotals(value) {
  ...
  if (typeof value.total !== "string" || !/^[1-9]\d{0,17}$/.test(value.total)) throw invalid("The monetary total is invalid.");
  ...
  if (typeof value.currency_code !== "string" || !/^[A-Z]{3}$/.test(value.currency_code)) throw invalid("The currency is invalid.");
  return { amountMinor, currency: value.currency_code };
}

function requireBoundSubject(store, customerId) {
  const subjectKey = store.resolveSubject(customerId);
...
function compareEventWithRow(event, row) {
  const currentPeriod = rowBillingPeriod(row);
  if (event.billingPeriod && currentPeriod) {
    const periodComparison = compareBillingPeriods(event.billingPeriod, currentPeriod);
    if (periodComparison !== 0) return periodComparison;
  }
  const currentOccurred = row.last_occurred_at ?? row.occurred_at;
  const timeComparison = Date.parse(event.occurredAt) - Date.parse(currentOccurred);
...
function compareBillingPeriods(first, second) {
  const end = Date.parse(first.endsAt) - Date.parse(second.endsAt);
  if (end !== 0) return Math.sign(end);
...
function requireExactCatalogObject(value, keys, label) {
...
function invalid(message) { return new InvalidSimulatorEventError(message); }
function hash(value) { return createHash("sha256").update(value, "utf8").digest("hex"); }
function stableId(prefix, ...parts) { return `${prefix}_${hash(parts.join(":")).slice(0, 24)}`; }
function hasStrongSecret(secret) { return typeof secret === "string" && Buffer.byteLength(secret, "utf8") >= minimumSecretBytes; }
function requireStrongSecret(secret) { if (!hasStrongSecret(secret)) throw new Error("The simulator secret must contain at least 32 UTF-8 bytes."); }
function requireRawBuffer(rawBody) { if (!Buffer.isBuffer(rawBody)) throw new TypeError("The webhook body must be the untouched raw Buffer."); }
