...
export function readSession(cookieHeader, secret, now = Date.now()) {
  ...
  try {
    ...
    return Number.isSafeInteger(session.expiresAt) && session.expiresAt > now ? session : {};
  } catch {
    return {};
  }
}
...
export function requireCsrf(request, session, applicationOrigin) {
  ...
  if (origin !== new URL(applicationOrigin).origin || typeof session.csrf !== "string" || typeof token !== "string") {
    throw new Error("invalid csrf");
  }
  const left = Buffer.from(session.csrf);
  const right = Buffer.from(token);
  if (left.length !== right.length || !timingSafeEqual(left, right)) throw new Error("invalid csrf");
}
...
function key(secret) {
  if (typeof secret !== "string" || secret.length < 32) throw new Error("APP_SESSION_SECRET must contain at least 32 characters");
